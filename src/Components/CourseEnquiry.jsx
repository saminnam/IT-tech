...
const CourseEnquiry = ({ isCourseopen, setIsCourseOpen, CourseTitle}) => {
  const [name, setName] = useState('');
  const [email, setEmail] = useState('');
  const [number, setNumber] = useState('');
  const [careerStatus, setcareerStatus] = useState('');
  const [course, setCourse] = useState(CourseTitle);
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        await axios.post('http://localhost:3000/api/courseenquiry', {
          name,
          email,
          number,
          careerStatus,
      ...
      <AnimatePresence>
        {isCourseopen && (
          <div className="fixed inset-0 flex items-center justify-center bg-gray-400/50">
            ...
            >
              <h2 className="text-2xl font-bold mb-6 text-center">Course Enquiry</h2>
              <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                <div>
                  <input
                    type="text"
                    placeholder="Course You Want to Know About"
                    value={course}
                    onBeforeInput={(e) => setCourse(e.target.value)}
                    className="w-full p-3 border rounded-lg focus:outline-none focus:ring-2 focus:ring-main-color"
                    readOnly
                  />
